...
void escribir_varios(double* vector1,int max,char* str1,int j,int n);
//void nombre (int max);
...
int main(int argc,char *argv[])
{
  ...
  */
  ...
  n = 1024;    // tamaño de la red
  z = 27000;  // cantidad de iteraciones
  iteracion_ns (n,z);
...
void iteracion_ns (int n, int z){
 ...
 float  prob;
 char   str1[50];
     int *vector_ns,*vector;
     double *promedio_vector_ns;
     vector_ns = (int *)malloc(n*n*sizeof(int));
     vector = (int *)malloc(n*n*sizeof(int));
     promedio_vector_ns = (double *)malloc(n*n*sizeof(int)); 
  ...
  { 
     ...
     for(m=0;m<n*n;m++){ //inicializo vectores a 0 en cada proba
	   ...
	   for(i=0;i<z;i++){
        ...
        for(m=0;m<n*n;m++){ 
          promedio_vector_ns[m] += (double)vector_ns[m]/z; 
        }        
	   }

	  sprintf(str1,"resultado1d_prob%f_n%i",prob,n);	
	  escribir_varios(promedio_vector_ns, max, str1, j,n);
  }	
}
